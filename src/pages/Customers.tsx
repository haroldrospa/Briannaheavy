      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="block w-full pl-11 pr-4 py-3 bg-white border-none rounded-full shadow-sm text-sm font-medium focus:ring-2 focus:ring-[#ED1C24]/20 transition-all" 
          />
        </div>
        <div className="w-full sm:w-56">
          <select className="block w-full px-4 py-3 bg-white border-none rounded-full shadow-sm text-sm font-medium focus:ring-2 focus:ring-[#ED1C24]/20 transition-all appearance-none cursor-pointer">
          ...
          <table className="min-w-full divide-y divide-gray-100">
            <thead>
              <tr>
                ...
                <th scope="col" className="px-6 py-5 text-left text-[11px] font-black text-gray-400 uppercase tracking-wider">Contacto</th>
                <th scope="col" className="px-6 py-5 text-left text-[11px] font-black text-gray-400 uppercase tracking-wider">Deuda / Crédito</th>
                ...
                <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-5 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-12 w-12 bg-[#f4f3f1] rounded-full flex items-center justify-center">
                        {customer.type === 'Empresarial' ? (
                          <BuildingOfficeIcon className="h-6 w-6 text-gray-400" />
                        ) : (
                          <span className="text-gray-500 font-bold">{customer.name.charAt(0)}</span>
                        )}
                      </div>
                      <div className="ml-4">
                        <div className="text-sm font-bold text-gray-900">{customer.name}</div>
                        <div className="text-[11px] font-bold uppercase tracking-wider text-gray-400 mt-1">{customer.type}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5 whitespace-nowrap">
                  ...
                  <td className="px-6 py-5 whitespace-nowrap">
                    <span className={`px-3 py-1 inline-flex text-xs font-bold rounded-full ${
                      customer.status === 'Activo' ? 'bg-green-100 text-green-800' : 
                      customer.status === 'En Mora' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                    }`}>
                      {customer.status}
                    </span>
                  </td>
                  <td className="px-6 py-5 whitespace-nowrap text-right text-sm font-medium">
                    <button className="text-[#ED1C24] hover:text-red-900 font-bold bg-red-50 px-4 py-2 rounded-full transition-colors hover:bg-red-100">Ver Perfil</button>
                  </td>
                </tr>
              ))}
            </tbody>
